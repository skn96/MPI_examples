PROGRAM Parallel_search
      ...
      ! common /pair/ gi, rval !Important to ensure that gi and rval are in consecutive memory locations
       ...
       call MPI_ADDRESS(gi, iadd0, err)
       iaddr = iadd0
       disp(1) = MPI_AINT_DIFF(iaddr, iadd0)
       call MPI_ADDRESS(rval, iaddr, err)
       disp(2) = MPI_AINT_DIFF(iaddr, iadd0)
       disp(1) = 0
